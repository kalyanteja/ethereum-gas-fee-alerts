...
exports.helloWorld = functions.https.onRequest(async (request, response) => {
    functions.logger.info("Hello there....checking ETH gas fee for you!");
    ...
    fetchUrl(check_gas_url, (error, meta, body) => {
        if (error) {
            ...
            if (std_gas_fee < 50) {
                ...
                response.send(`Twilio alert sent successfully. Base Gas Fee one must pay: ${std_gas_fee} GWei`);
            } else {
                response.send(`No need to send an alert. Fees are ridiculously high: ${std_gas_fee} GWei`);
            }
        }
    });
